...
class RNNClassifier:
    def __init__(self, iPara):
        ...
        self.n_z = 2
        self.OutName = 'txtLog/256_lr1e3_RNNSep' + str(iPara) + '.txt'
    ...
    def _initialize_weights(self):
        ...
        all_weights['weights_gener'] = {
            'h1': tf.Variable(xavier_init(self.num_hidden, self.n_hidden_gener_1)),
    ...
    def _generator_network_0(self, weights, biases, state):
        layer_1 = self.transfer_fct(tf.add(tf.matmul(state, weights['h1']), biases['b1'])) 
        layer_2 = self.transfer_fct(tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])) 
        hz_softmax = tf.nn.softmax(tf.add(tf.matmul(layer_2, weights['out_mean']), biases['out_mean']), 1)
        return hz_softmax

    def _generator_network(self, weights, biases, state):
        layer_1 = self.transfer_fct(tf.add(tf.tensordot(state, weights['h1'], axes=((2,), (0,))), biases['b1'])) 
    ...
    def _create_loss(self):
        ...
        self.lossRNN = self.reconstr_loss
    ...
    def _create_optimizer(self):
        with tf.name_scope("train"):
            ...
            self.training_opRNN = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.lossRNN)
            ...
            for step in range(initial_step, initial_step + self.num_train_steps):
                ...
                for batch_idx, (batch_start, batch_end) in enumerate(training_batches):
                    ...
                    _, batch_loss, = sess.run(
                        [self.training_opRNN, self.lossRNN],
                        feed_dict=feed_dict)

                    loss_per_batch += batch_loss / len(training_batches)

                step_time = (time.time() - start_time)
                print("global step %d/%d step-time %.2fs average loss %.5f" % (step, self.num_train_steps - 1, step_time, loss_per_batch))
            ###VAE End
