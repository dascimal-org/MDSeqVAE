...
class RNNClassifier:
    def __init__(self, iPara):
        ...
        self.n_z = 4096
        self.OutName = 'txtLog/z4096_KLC001VAESep' + str(iPara) + '.txt'
    ...
    def _create_loss(self):
        ...
        with tf.name_scope("loss_vae"):
            z_log_sigma_sq0 = self.z_log_sigma_sq[:(self.batch_size//2),:]
            z_sigma_sq0 = tf.exp(z_log_sigma_sq0)
            prior_0_sigma_sq = tf.exp(self.prior_0_log_sigma_sq)
            z_mean0 = self.z_mean[:(self.batch_size//2),:]
            self.loss_z_0 =  tf.reduce_mean(0.5 * tf.reduce_sum(z_log_sigma_sq0 - self.prior_0_log_sigma_sq + (prior_0_sigma_sq + tf.square(z_mean0 - self.prior_0_mean))  / (z_sigma_sq0) - 1, 1))

            z_log_sigma_sq1 = self.z_log_sigma_sq[-(self.batch_size//2):,:]
            z_sigma_sq1 = tf.exp(z_log_sigma_sq1)
            prior_1_sigma_sq = tf.exp(self.prior_1_log_sigma_sq)
            z_mean1 = self.z_mean[-(self.batch_size//2):,:]
            self.loss_z_1 =  tf.reduce_mean(0.5 * tf.reduce_sum(z_log_sigma_sq1 - self.prior_1_log_sigma_sq + (prior_1_sigma_sq + tf.square(z_mean1 - self.prior_1_mean))  / (z_sigma_sq1) - 1, 1))

            self.loss_prior_0_1 = -0.5 * tf.reduce_sum(self.prior_1_log_sigma_sq - self.prior_0_log_sigma_sq + (prior_0_sigma_sq + tf.square(self.prior_1_mean - self.prior_0_mean)) / (prior_1_sigma_sq) - 1)
        ...
        # self.lossVAE = 0.15 * self.loss_prior_0_1 + 0.1 * self.loss_z_0 + 0.1 * self.loss_z_1 + 0.05 * self.reconstr_loss
        self.lossVAE = 0.0001 * self.loss_prior_0_1 + 0.0007 * self.loss_z_0 + 0.0007 * self.loss_z_1 + 0.008 * self.reconstr_loss
        self.loss = 1.0 * self.loss_svm
    ...
    def _create_optimizer(self):
        with tf.name_scope("train"):
            ...
            self.training_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.loss, var_list=[self.w_rf])
            self.training_opVAE = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.lossVAE)
            ...
            for step in range(initial_step, initial_step + self.num_train_steps):
                ...
                for batch_idx, (batch_start, batch_end) in enumerate(training_batches):
                    ...
                    _, batch_loss, = sess.run(
                        [self.training_opVAE, self.lossVAE],
                        feed_dict=feed_dict)
                ...
                print("global step %d/%d step-time %.2fs average loss %.5f" % (step, self.num_train_steps - 1, step_time, step_loss))
            ###VAE End
