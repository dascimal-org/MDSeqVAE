...
class RNNClassifier:
    def __init__(self, iSize):
        ...
        self.n_z = 2
        self.iSize = iSize
        self.OutName = 'txtLog/256_lr1e3_RNNConcat' + str(self.iSize)  + '.txt'
    ...
    def _create_oc_svm(self):
        with tf.name_scope("oc-svm"):
            ...
            #phi_x_tilde = self.z
            self.phi_x_tilde = tf.reshape(self.outputs, [self.batch_size, -1])

            self.w_rf = tf.Variable(tf.truncated_normal((self.phi_x_tilde.get_shape().as_list()[1], 1), stddev=0.05),
                                    name='w_rf')  # (2*n_random_features, 1)

            # self.predict = tf.nn.tanh(tf.matmul(phi_x_tilde, self.w_rf))  # (batch_size, 1)
            self.predict = tf.matmul(self.phi_x_tilde, self.w_rf)  # (batch_size, 1)
...
def main():
    ...
    if model.running_mode == 1:  # train
        ...
        x_train_opcode, x_train_assembly, x_train_seq_len, y_train, \
        x_valid_opcode, x_valid_assembly, x_valid_seq_len, y_valid, \
        x_test_opcode, x_test_assembly, x_test_seq_len, y_test, model.time_steps, model.vocab_opcode_size = utils.load_vul_deepackerFixedLength(list_arch_os, ourLength)

        print(model.time_steps)
        print(np.mean(x_train_seq_len))
        print(np.mean(x_valid_seq_len))
        print(np.mean(x_test_seq_len))
        print(x_train_opcode.shape)
        print(x_train_assembly.shape)
        print(x_valid_opcode.shape)
        print(x_valid_assembly.shape)
        print(x_test_opcode.shape)
        print(x_test_assembly.shape)
        # sys.exit()
