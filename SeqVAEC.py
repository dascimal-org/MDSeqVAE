...
class RNNClassifier:
    def __init__(self, iPara):
        ...
        self.wp01 = 0.2
        self.wrecon = 0.008
        self.OutName = 'txtLog/z4096_O995_Lr0_001_VAEOri' + str(iPara) + '.txt'
    ...
    def _create_loss(self):
        ...
        with tf.name_scope("loss_vae"):
            ...
            self.latent_loss = tf.reduce_mean(-0.5 * tf.reduce_sum(1 + self.z_log_sigma_sq - tf.square(self.z_mean) - tf.exp(self.z_log_sigma_sq), 1))
        ...
        self.loss = self.wsvm * self.loss_svm + self.wp01 * self.latent_loss + self.wrecon * self.reconstr_loss
